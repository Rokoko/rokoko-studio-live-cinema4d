...
#
# They share most parameters, where one half is usually passed directly from
# GetDDescription.
# The other half should be pretty self explanatory options for the created
# widgets.

def GetDDescriptionCreateCombo(
    node,
    description,
    singleId,
    id,
...
def GetDDescriptionCreateLong(
    ...
    '''Create a long parameter with edit widget, optionally additonal slider
    widget.
    '''
...
def GetDDescriptionCreateReal(
    ...
    '''Create a real/float parameter with edit widget, optionally additional
    slider widget.
    '''
...
def GetDDescriptionCreateVector(
    node,
    description,
    singleId,
    id,
    ...
    '''Create a Vector parameter with edit widget, optionally with color
    CustomGUI instead
    '''
...
def GetDDescriptionCreateGroup(
    node,
    description,
    singleId,
    id,
    ...
    '''Create a widget group, its id can be used as parendId for other
    parameters/widgets.
    '''
    ...
    return description.SetParameter(
        descId, bc, c4d.DescID(c4d.DescLevel(parentId)))


def GetDDescriptionCreateButton(
    node,
    description,
    singleId,
    id,
    ...
    In C4D buttons in Descriptions are a bit different, as they don't end up
    in SetDParameter() or any other of the usual Description messages, but need
    to be handled in Message(MSG_DESCRIPTION_COMMAND).
    '''
...
def GetDDescriptionCreateString(
    node,
    description,
    singleId,
    id,
    ...
    '''Create a string parameter with edit widget, optionally with static text
    widget instead.
    '''
...
def GetDDescriptionCreateLink(
    ...
    descId = c4d.DescID(
        c4d.DescLevel(id, c4d.DTYPE_BASELISTLINK, node.GetType()))
    if singleId is not None and not descId.IsPartOf(singleId)[0]:
        return True
