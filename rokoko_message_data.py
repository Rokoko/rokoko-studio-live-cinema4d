'''In Cinema 4D a MessageData plugin has no real function itself. As the name
suggests it's main purpose is to receive messages. In contrast to dialogs it
does so all the time (C4D's runtime, while a dialog needs to be open to receive
messages).

Messages are received in CoreMessage(), which is guranteed to be called from
the main thread.

In the context of Rokoko Studio Live it is a central component, owning the
listener thread (well, it's the listener thread is a global resource,
so it's more in a sense of this MessageData (mainly) controlling the
listener thread) and being the listener's event interface.
Receiving events for example to control the player and sending around
status and update events.

The viewport updates during playback are also done here, basically in reaction
to an event message caused by a frame dispatch.
'''
...
# Created by rokoko_listener during initialization
g_thdListener = GetListenerThread()
...
class MessageDataRokoko(c4d.plugins.MessageData):
    _docLast = None
    # State of auto connect during startup
    # 0: auto connect not done, yet
    # 1: auto connect in progress
    # 2: auto connect done
    _init = 0
    ...
    def StartAutoConnect(self):
        ...
        # TODO: Not sure yet, how the connected dataset survives shutdown
        #       (it's deleted in PluginMessage END_ACTIVITY...)
        # Anyway, we'll simply throw any remaining connected data set away.
        bcConnected = GetConnectedDataSet()
        if bcConnected is not None:
            RemoveConnectedDataSet()

        # Connect the first connection with auto connect enabled
        # (and only this, in case there should be multiple by mistake)
        bcConnections = GetPrefsContainer(rid.ID_BC_CONNECTIONS)
        for id, bcConnection in bcConnections:
            if not bcConnection[rid.ID_BC_DATASET_LIVE_AUTOCONNECT]:
                continue
    ...
    def FinishAutoConnect(self):
        '''Upon "connection status change" message, an auto connect in progress
        is finalized here
        '''

        self._init = 2  # auto connect done

        # Have all tags recognize the new connection (e.g. to reflect new
        # actors,...)
        tags = GetTagList()
        for tag in tags:
            tag.Message(c4d.MSG_MENUPREPARE)

    # Counter _cntUpdateDlgFrameCount is used here in CoreMessageLiveDraw(),
    # only.
    # Its purpose is to reduce the number of update events send to the player
    # to a "reasonable yet good looking" amount.
    _cntUpdateDlgFrameCount = 0

    def CoreMessageLiveDraw(self):
        '''CoreMessageLiveDraw() is the reaction to a "live draw request"
        message, which is sent after frames got dispatched to tags.

        This will trigger C4D's execution pipeline and scene evaluation,
        resulting in Execute() calls to our (not only, actually all) tags,
        followed by a viewport update.
        '''

        # Depending on "Playback Rate" set by the user, send the current
        # frame number only every other frame to the Manager dialog.
    ...
    def CoreMessageConnect(self, bc):
        ...
        SetConnectedDataSet(GetPrefsContainer(
            rid.ID_BC_CONNECTIONS)[idConnection])
        g_thdListener.Connect()
    ...
    def CoreMessageStartListening(self):
        ...
        # Backup state of all objects we'll be messing with during playback
        # (plus current document time)
        doc = c4d.documents.GetActiveDocument()
    ...
    def CoreMessagePlay(self, bc):
        ...
        c4d.SpecialEventAdd(rid.PLUGIN_ID_COREMESSAGE_MANAGER,
                            rid.CM_SUBID_MANAGER_PLAYBACK_STATUS_CHANGE)
        ...
        # All involved tags have their execution flag set, in order to have
        # their Execute() function actually do something.
        tagsLive = g_thdListener.GetTagConsumers()
        for tag in tagsLive:
            # Avoid SetParameter
            tag.GetDataInstance()[rid.ID_TAG_EXECUTE_MODE] = 1
    ...
    def CoreMessagePause(self, bc):
        ...
        if g_thdListener._play:
            ...
            # Player no longer plays incoming live stream
    ...
    def CoreMessageStop(self):
        ...
        c4d.SpecialEventAdd(
            rid.PLUGIN_ID_COREMESSAGE_MANAGER_CURRENT_FRAME_NUMBER, 0, 0)
        # Inform Manager dialog about changed player status
        c4d.SpecialEventAdd(rid.PLUGIN_ID_COREMESSAGE_MANAGER,
                            rid.CM_SUBID_MANAGER_PLAYBACK_STATUS_CHANGE)
    ...
    def CoreMessageConnectionStatusChange(self):
        ...
        Actually a message received by the Manager dialog to update connection
        status.
        Here it's only used to finalize the auto connection process.
        '''
    ...
    def CoreMessageLiveDataChange(self):
        ...
        # (e.g. new actors may become available or old ones might have gone
        # missing)
        tags = GetTagList()
        for tag in tags:
            tag.Message(rid.PLUGIN_ID_MSG_DATA_CHANGE)
    ...
    def CoreMessageEMsgChange(self):
        ...
        In this plugin it's used only to detect if the current document has
        changed (talking about a different document, NOT a change within
        a document).
        '''

        # Check if document has changed
        docCurrent = c4d.documents.GetActiveDocument()
        if self._docLast is not None and self._docLast.IsAlive() and \
           docCurrent == self._docLast:
        ...
        # Inform Manager dialog about document change (it will need to collect
        # all tags, for example)
        c4d.SpecialEventAdd(rid.PLUGIN_ID_COREMESSAGE_MANAGER,
                            rid.CM_SUBID_MANAGER_UPDATE_TAGS)

        # Reinitialize all tags.
        # Several reasons could cause a need for a reinitialization,
        # just a few examples:
    ...
    def CoreMessage(self, id, bc):
        ...
        # If not done so already, initate auto connect
        # (happens only once during startup)
        if self._init == 0:
        ...
        if id == c4d.EVMSG_CHANGE:
        ...
        elif id == rid.PLUGIN_ID_COREMESSAGE_LIVE_DRAW:
            self.CoreMessageLiveDraw()

        elif id == rid.PLUGIN_ID_COREMESSAGE_CONNECTION:
            subId = GetCoreMessageParam(bc)
            if subId == rid.CM_SUBID_CONNECTION_CONNECT:
                self.CoreMessageConnect(bc)
            elif subId == rid.CM_SUBID_CONNECTION_DISCONNECT:
                self.CoreMessageDisconnect()
            elif subId == rid.CM_SUBID_CONNECTION_STATUS_CHANGE:
            ...
            subId = GetCoreMessageParam(bc)
            if subId == rid.CM_SUBID_PLAYER_START:
                self.CoreMessageStartListening()
            elif subId == rid.CM_SUBID_PLAYER_PAUSE_RECEPTION:
                self.CoreMessagePauseListening()
            elif subId == rid.CM_SUBID_PLAYER_EXIT:
                self.CoreMessageStopListening()
            elif subId == rid.CM_SUBID_PLAYER_PLAY:
                self.CoreMessagePlay(bc)
            elif subId == rid.CM_SUBID_PLAYER_PAUSE:
                self.CoreMessagePause(bc)
            elif subId == rid.CM_SUBID_PLAYER_STOP:
                self.CoreMessageStop()
            elif subId == rid.CM_SUBID_PLAYER_FLUSH_LIVE_BUFFER:
